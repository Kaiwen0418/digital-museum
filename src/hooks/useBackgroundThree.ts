...
import { DEVICES, MODEL_CONFIGS } from '../data/devices';
import { clamp, smoothstep } from '../lib/math';
import type { ModelConfig, ProgressCanvas } from '../types/device';
...
function createDeviceMesh(id: string, darkMode: boolean) {
  ...
  const material = new THREE.MeshStandardMaterial({ color: colorMap[id], roughness: 0.42, metalness: 0.24 });
...
function tuneModelMaterials(root: any, darkMode: boolean, modelConfig?: ModelConfig) {
  ...
  root.traverse((child: any) => {
    ...
    materials.forEach((material: any) => {
      ...
      if (typeof material.roughness === 'number') material.roughness = Math.max(0.18, material.roughness * roughnessScale);
      if (typeof material.metalness === 'number') material.metalness = Math.min(0.22, material.metalness * metalnessScale);
...
export function useBackgroundThree(canvasRef: RefObject<ProgressCanvas | null>, progress: number, darkMode: boolean) {
    ...
    DEVICES.forEach((item, idx) => {
      ...
      loader.load(
        ...
        (gltf: any) => {
          ...
          model.scale.setScalar(modelConfig.scale / maxDim);
